...
# This program simulates Breadth-first search on following example
...
class BFS:
    ...
    # Iterates over Open queue
    def iterate(self):
        while(self.found() != True):
            # Open queue is empty and result has not been found
            if(len(self.open) == 0):
                print("Open is empty, goal was not found")
                return
            self.use_rules()
            # Adding expanded node to Closed queue
            self.closed.append(self.open[0])
            # Popping expanded node
            self.open.pop(0)
        
        # Adding goal node to the closed queue
        self.closed.append(self.open[0])
        self.open.pop(0)
    ...
    # Checks if created node is in Open or Close queue
    ...
    def use_rules(self):
        ...
        # Appends new sequences created by rules to Open queue if it is not contained Closed or Open already
    ...
    def found(self):
        i = 0
        for i in range(0,len(self.open[0])-2):
            if(type(self.open[0][i]) is not type(self.goal[i])):
                return False          
        return True
